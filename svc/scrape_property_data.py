...
def main(event, context):
    ...
    """
    logger = setup_logger(
        "scrape-property-data", include_timestamp=False, propagate=False
    )

    output_bucket = os.environ["OUTPUT_BUCKET"]
    html_file_fetched_at = event["timestamp"]
    raw_data_key = event["raw_data_key"]
    base, _, _ = raw_data_key.rpartition(".zip")
    scraped_data_key = (
        f"{base}.pickle" if base else raw_data_key.replace(".zip", ".pickle")
    )
    ...
    with io.BytesIO() as stream:
        s3_client.download_fileobj(
            Bucket=output_bucket, Key=raw_data_key, Fileobj=stream
        )
        with zipfile.ZipFile(stream) as zfile:
            filenames = sorted(
                (zi for zi in zfile.infolist()), key=lambda zi: zi.filename
            )
        # TODO: joblib runs in sequential even for n_jobs > 1 due to limitations
        #   of multiprocessing module in AWS Lambda
        properties = scrape_properties_from_files(
            filenames, stream, logger=logger, n_jobs=1
        )
    ...
    with io.BytesIO() as stream:
        ...
        s3_client.upload_fileobj(
            Fileobj=stream, Bucket=output_bucket, Key=scraped_data_key
        )
